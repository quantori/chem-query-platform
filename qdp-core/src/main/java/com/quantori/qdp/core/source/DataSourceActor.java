...
class DataSourceActor<S extends SearchItem, I extends StorageItem>
  ...
  static <S extends SearchItem, I extends StorageItem> Behavior<Command> create(
    ...
    return Behaviors.setup(
        ctx -> new DataSourceActor<>(ctx, searchIterators, searchRequest, bufferActorSinkRef));
  }
  ...
  private Behavior<Command> onStatusFlow(StatusFlow cmd) {
    cmd.replyTo.tell(
        StatusReply.success(
            new StatusResponse(
                sourceFetchComplete.get(),
                new ArrayList<>(errors),
                foundByStorageCount.get(),
                matchedCount.get())));
    return this;
  }
  ...
  private Source<S, NotUsed> getSource() {
    ...
    List<SearchIterator<I>> dataSearcherList =
        searchIterators.values().stream().flatMap(Collection::stream).toList();
    if (dataSearcherList.size() == 1) {
      return getSource(
          dataSearcherList.get(0),
          parallelism,
          searchRequest.getResultFilter(),
          searchRequest.getResultTransformer());
      ...
      List<Source<S, ?>> remainIterators =
          dataSearcherList.subList(2, dataSearcherList.size()).stream()
              .map(
                  dataSearcher ->
                      getSource(
                          dataSearcher,
  ...
  private Source<S, NotUsed> getSource(
      SearchIterator<I> searchIterator,
      int parallelism,
      Predicate<I> resultFilter,
      Function<I, S> resultTransformer) {
    Source<I, NotUsed> source =
        Source.unfoldResource(
            ...
            .mapConcat(list -> list)
            .withAttributes(
                ActorAttributes.withSupervisionStrategy(Supervision.getStoppingDecider()));
    return addFlowStep(
        searchIterator.getStorageName(),
        searchIterator.getLibraryIds(),
        parallelism,
        source,
        resultFilter,
        resultTransformer);
  ...
  private Predicate<I> filterStep(
    ...
    return item -> {
      try {
        ...
        errors.add(
            new SearchError(
                ErrorType.FILTER, storageName, libraryIds, "Molecule filter step failed"));
        throw e;
      }
    };
  }
      ...
      try {
        ...
        errors.add(
            new SearchError(
                ...
                storageName,
                libraryIds,
                "Molecule transformation step failed"));
        throw e;
      }
    };
  }
