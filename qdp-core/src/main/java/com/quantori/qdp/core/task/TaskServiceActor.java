...
public class TaskServiceActor extends AbstractBehavior<TaskServiceActor.Command> {
    ...
    private Behavior<Command> onCreateTask(CreateTask createTaskCmd) {
        ActorRef<StreamTaskActor.Command> taskRef = getContext().spawn(
                StreamTaskActor.create(createTaskCmd.taskPersistenceService(),
                        createTaskCmd.replyTo), "task-" + UUID.randomUUID());
    ...
    private Behavior<Command> onResumeTask(ResumeTask resumeTaskCmd) {
        ActorRef<StreamTaskActor.Command> taskRef = getContext().spawn(
                StreamTaskActor.create(resumeTaskCmd.taskPersistenceService(), resumeTaskCmd.taskId, resumeTaskCmd.replyTo),
    ...
    private Behavior<Command> onCreateFlow(CreateFlow createFlowCmd) {
        ActorRef<StreamTaskActor.Command> taskRef = getContext().spawn(
                TaskFlowActor.create(createFlowCmd.service(), createFlowCmd.taskPersistenceService(),
                        createFlowCmd.type(), createFlowCmd.replyTo),
    ...
    private Behavior<Command> onResumeFlow(ResumeFlow resumeFlowCmd) {
        ActorRef<StreamTaskActor.Command> taskRef = getContext().spawn(
                TaskFlowActor.create(resumeFlowCmd.service(), resumeFlowCmd.taskPersistenceService(),
                        resumeFlowCmd.flowId(), resumeFlowCmd.type(), resumeFlowCmd.replyTo),
    ...
    public record CreateTask(
            ...
            TaskPersistenceService taskPersistenceService) implements Command {
    }

    public record ResumeTask(
            ActorRef<StatusReply<ActorRef<StreamTaskActor.Command>>> replyTo,
            TaskPersistenceService taskPersistenceService,
            String taskId) implements Command {
    }
