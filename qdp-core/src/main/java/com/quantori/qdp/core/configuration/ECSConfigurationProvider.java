...
public class ECSConfigurationProvider {
    ...
    try {
      ...
      DescribeTasksResult result =
          client.describeTasks(
              new DescribeTasksRequest().withCluster(clusterArn).withTasks(taskArn));

      Task taskDetails = result.getTasks().stream().findFirst().orElseThrow();
  ...
  private static String getTaskArn(JsonNode taskMetadata) {
    return taskMetadata.get(LABELS).get(TASK_ARN_PROPERTY_NAME).asText();
  }

  private static String getClusterArn(JsonNode taskMetadata) {
    return taskMetadata.get(LABELS).get(CLUSTER_ARN_PROPERTY_NAME).asText();
  }

  private static String getHostIp(Task taskDetails) {
    return taskDetails.getContainers().get(0).getNetworkInterfaces().get(0).getPrivateIpv4Address();
  }

  private static String getServiceName(Task taskDetails) {
    return taskDetails.getGroup().split("service:")[1];
  }

  private static String getClusterName(Task taskDetails) {
    return taskDetails.getClusterArn().split("cluster/")[1];
  }
  ...
  private static JsonNode getTaskMetadata(String metadataUri)
    ...
    HttpResponse<String> response =
        httpClient.send(
            HttpRequest.newBuilder(new URI(metadataUri))
