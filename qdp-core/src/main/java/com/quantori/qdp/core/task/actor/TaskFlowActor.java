...
public class TaskFlowActor extends StreamTaskActor {
    ...
    private Behavior<Command> onAdjustPercent(AdjustPercent cmd) {
        logger.trace("Flow percent {} was updated  {}, from task {}", taskId, cmd.percent, cmd.taskNumber);
    ...
    private Behavior<Command> onGetFlowResult(StreamTaskActor.GetTaskResult cmd) {
        ...
        switch (status) {
            ...
            case COMPLETED_WITH_ERROR ->
                    cmd.replyTo().tell(StatusReply.error(new StreamTaskProcessingException("The flow was failed")));
            default ->
                    cmd.replyTo().tell(StatusReply.error(new StreamTaskProcessingException("Flow is not completed yet")));
        }
        return Behaviors.withTimers(timer -> {
            timer.startSingleTimer(timerId, new Timeout(), timeout);
            return this;
    ...
    private Behavior<Command> onTimeoutSubTasks(Command cmd) {
        ...
        tasksIds.keySet().forEach(num -> {
            try {
                closeTaskByNumber(num);
            } catch (RuntimeException e) {
                logger.warn("Cannot close a sub task {} (task id: {}) in the flow by timeout.", num, tasksIds.get(num), e);
            }
        });
        ...
        if (user.equals(cmd.user())) {
            ...
            tasksIds.keySet().forEach(num -> {
                try {
                    ...
                    logger.warn("Cannot close a sub task {} (task id: {}) in the flow.", num, tasksIds.get(num), e);
                }
            });
            return Behaviors.stopped();
        } else {
    ...
    private Behavior<Command> onCancel(Cancel cmd) {
        ...
        if (!status.equals(StreamTaskStatus.Status.COMPLETED) && !status.equals(StreamTaskStatus.Status.COMPLETED_WITH_ERROR)) {
            logger.trace("Flow was canceled {}", taskId);
            this.status = StreamTaskStatus.Status.COMPLETED_WITH_ERROR;
            tasksIds.keySet().stream().sorted(Comparator.reverseOrder()).forEach(num -> {
                try {
                    var taskId = tasksIds.get(num);
                    service.cancelTask(taskId, user);
                } catch (RuntimeException e) {
                    logger.warn("Cannot cancel a sub task {} (task id: {}) in the flow.", num, tasksIds.get(num), e);
                }
            });
            persistFlowState();
        }
    ...
    private Behavior<Command> onStartFlow(Command cmd) {
        ...
        if (status.equals(StreamTaskStatus.Status.INITIATED)) {
            ...
            if (cmd instanceof StartTask cmdStartTask) {
                ...
                taskWeights = normalizeTaskWeights(cmdStartFlow.streamTaskDescriptions().stream().map(StreamTaskDescription::getWeight).collect(Collectors.toList()));
    ...
    private Behavior<Command> onSubTaskComplete(SubTaskComplete cmd) {
        ...
        if (!statuses.containsKey(cmd.taskNumber())) {
            try {
                ...
                if (!noMoreTask() && cmd.success()
                        && status.equals(StreamTaskStatus.Status.IN_PROGRESS)) {
    ...
    private void resumeSubTask(String subTaskId, StreamTaskResult lastTaskResult) {
        TaskStatus subtaskStatus = persistenceService.grabSubTaskStatus(
    ...
    private CompletionStage<StreamTaskStatus> resumeTask(StreamTaskDescription task, String subTaskId) {
        return persistenceService.resumeTaskCommand(UUID.fromString(subTaskId))
                .thenCompose(actorRef -> persistenceService.startTaskCommand(actorRef, task, taskId, parallelism, buffer));
    }

    private void reportCompletedTask(boolean successful, int taskNumber, StreamTaskResult flowResult) {
        @SuppressWarnings("unused")
        CompletionStage<StreamTaskStatus> result = AskPattern.askWithStatus(
                getContext().getSelf(),
    ...
    private void persistFlowState() {
        try {
            persistenceService.persistFlowState(onComplete,
                    descriptions,
                    currentTaskNumber,
                    ...
                    status,
                    type,
                    user, parallelism, buffer);
        } catch (RuntimeException e) {
            logger.debug("Cannot persist a flow state: {} started by user {}", taskId,
                    user, e);
        }
    }
    ...
    public record StartFlow(
            ...
            String user, int parallelism, int buffer) implements Command {
    }
    ...
    private record SubTaskComplete(
            ActorRef<StatusReply<StreamTaskStatus>> replyTo, int taskNumber,
            boolean success, StreamTaskResult taskResult, Throwable error) implements Command {
    ...
    class ResultAggregatorWrapper implements ResultAggregator {
        ...
        public void taskCompleted(boolean successful) {
            try {
                ...
                logger.error("The error has been caught for a task completion in the flow {}", TaskFlowActor.this.taskId, e);
            }
            try {
                reportCompletedTask(successful, taskNumber, getResult());
            } catch (RuntimeException e) {
                logger.error("The report about the task completion was failed in the flow {}", TaskFlowActor.this.taskId, e);
            }
        }
    }
