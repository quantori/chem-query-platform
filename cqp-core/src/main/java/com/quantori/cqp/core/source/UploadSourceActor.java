...
class UploadSourceActor<D extends DataUploadItem, U extends StorageUploadItem>
  ...
  private UploadSourceActor(
      ActorContext<Command> context, DataStorage<U, ?> storage, int maxUploadCount) {
    super(context);
    if (maxUploadCount <= 0) {
  ...
  static <U extends StorageUploadItem> Behavior<Command> create(
      DataStorage<U, ?> storage, int maxUploads) {
    ...
    return completedStage(command)
        .thenComposeAsync(
            cmd -> {
              ...
              return loader
                  .loadStorageItems(dataSource, cmd.transformation, itemWriter::write)
  ...
  @AllArgsConstructor
  static class LoadFromDataSource<D extends DataUploadItem, U extends StorageUploadItem>
      implements Command {
