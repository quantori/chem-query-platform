...
public class StreamTaskActor extends AbstractBehavior<StreamTaskActor.Command> {
  ...
  public static Behavior<Command> create(
      TaskPersistenceService persistenceService, ActorRef<StatusReply<ActorRef<Command>>> replyTo) {
    return Behaviors.setup(
        ctx ->
            Behaviors.withTimers(
                timer -> new StreamTaskActor(ctx, timer, persistenceService, replyTo)));
  }
  ...
  public static Behavior<Command> create(
    ...
    return Behaviors.setup(
        ctx ->
            Behaviors.withTimers(
                timer -> new StreamTaskActor(ctx, timer, persistenceService, taskId, replyTo)));
  }
  ...
  private void registerSearchActor(
    ...
    var listener =
        Behaviors.receive(Receptionist.Registered.class)
            ...
            .build();
    var refListener =
        getContext().spawn(listener, "task-registerer-" + UUID.randomUUID().toString());

    // Register this actor in Receptionist to make it globally discoverable.
    context
        .getSystem()
        .receptionist()
        .tell(Receptionist.register(serviceKey, context.getSelf(), refListener));
  }
  ...
  private Behavior<Command> onStart(StartTask startTask) {
    ...
    logger.info(
        "Start task initial request with taskId: {}, by user {}, in flow: {}, task type: {}",
        taskId,
        taskDescription.getUser(),
        flowId,
    ...
    if (this.status.equals(StreamTaskStatus.Status.INITIATED)) {
      ...
      try {
        ...
        logger.error(
            "Error to start task with Id: {}, cmd : {} (user: {})",
            taskId,
            startTask,
            streamTaskDescription.getUser(),
            e);
        status = StreamTaskStatus.Status.COMPLETED_WITH_ERROR;
        completed(false);
      }
      persistTaskState();
      startTask.replyTo.tell(StatusReply.success(new StreamTaskStatus(taskId, status, 0)));
    }
  ...
  protected void registerForUser(String user) {
    var userKey = ServiceKey.create(Command.class, Objects.requireNonNull(user));

    getContext()
        .getSystem()
        .receptionist()
        .tell(Receptionist.register(userKey, getContext().getSelf()));
  }
    ...
    try {
      if (streamTaskDescription instanceof ResumableTaskDescription resumableTaskDescription) {
        persistenceService.persistTask(
            UUID.fromString(taskId), status, resumableTaskDescription, flowId, parallelism, buffer);
      }
    } catch (RuntimeException e) {
      logger.debug(
          "Cannot persist a task state: {} started by user {}",
          taskId,
          streamTaskDescription.getUser(),
          e);
    }
  ...
  Behavior<Command> onStatus(GetStatus getStatus) {
    ...
    if (Objects.nonNull(streamTaskDescription)
        && !getStatus.user.equals(streamTaskDescription.getUser())) {
      logger.warn(
          "Task details access violation for user: {}, taskId {}", getStatus.user(), taskId);
      getStatus.replyTo.tell(
          StatusReply.error(
    ...
    getStatus.replyTo.tell(
        StatusReply.success(
            new StreamTaskStatus(taskId, status, type, stage, percent, stagePercent, messages)));
    return Behaviors.withTimers(
        timer -> {
          timer.startSingleTimer(timerId, new Timeout(), timeout);
          return this;
  ...
  private Behavior<Command> onTaskResult(GetTaskResult getTaskResult) {
    ...
    if (!getTaskResult.user.equals(streamTaskDescription.getUser())) {
      logger.warn(
          "Task result access violation for user: {}, taskId {}", getTaskResult.user(), taskId);
      getTaskResult.replyTo.tell(
          StatusReply.error(
              new StreamTaskProcessingException(
    ...
    switch (status) {
      case COMPLETED ->
          getTaskResult.replyTo.tell(
              StatusReply.success(streamTaskDescription.getAggregator().getResult()));
      case COMPLETED_WITH_ERROR ->
          getTaskResult.replyTo.tell(
              StatusReply.error(new StreamTaskProcessingException("Task execution was failed")));
      default ->
          getTaskResult.replyTo.tell(
              StatusReply.error(new StreamTaskProcessingException("Task is not ready")));
    }
    return Behaviors.withTimers(
        timer -> {
          timer.startSingleTimer(timerId, new Timeout(), timeout);
  ...
  private Behavior<Command> onTaskDetails(GetTaskDetails getTaskDetails) {
    ...
    if (!getTaskDetails.user.equals(streamTaskDescription.getUser())) {
      logger.warn(
          "Task details access violation for user: {}, taskId {}", getTaskDetails.user(), taskId);
      getTaskDetails.replyTo.tell(
          StatusReply.error(
              new StreamTaskProcessingException(
    ...
    getTaskDetails.replyTo.tell(
        StatusReply.success(
            new StreamTaskDetails(
                taskId, type, stage, streamTaskDescription.getUser(), created, details, status)));
  ...
  private Behavior<Command> onCancel(Cancel cmd) {
    logger.info(
        "Stream task was canceled with taskId {}, user: {}",
        taskId,
        streamTaskDescription.getUser());
    if (streamTaskDescription.getUser().equals(cmd.user())) {
  ...
  private Behavior<Command> onComplete(StreamCompleted cmd) {
    logger.info(
        "Stream task completed with taskId {}, user: {}, status: {}",
        taskId,
        streamTaskDescription.getUser(),
        status);
    if (status.equals(StreamTaskStatus.Status.COMPLETED_WITH_ERROR)) {
  ...
  private Behavior<Command> onItem(Item element) {
    ...
    if (status.equals(StreamTaskStatus.Status.IN_PROGRESS)) {
      try {
        ...
        logger.error(
            "Error in task {} started by user {}", taskId, streamTaskDescription.getUser(), e);
        status = StreamTaskStatus.Status.COMPLETED_WITH_ERROR;
  ...
  private Behavior<Command> onFailure(StreamFailure failed) {
    logger.error(
        "Task stream failed! Task id: {}, User: {}",
        taskId,
        streamTaskDescription.getUser(),
        failed.cause);
    completed(false);
  ...
  public record GetStatus(ActorRef<StatusReply<StreamTaskStatus>> replyTo, String user)
      implements Command {}

  private record StreamInitialized(ActorRef<Ack> replyTo) implements Command {}

  public record GetTaskResult(ActorRef<StatusReply<StreamTaskResult>> replyTo, String user)
      implements Command {}

  public record GetTaskDetails(ActorRef<StatusReply<StreamTaskDetails>> replyTo, String user)
      implements Command {}

  private static class StreamCompleted implements Command {}

  private record Item(ActorRef<Ack> replyTo, DataProvider.Data data) implements Command {}
  ...
  public record Cancel(ActorRef<StatusReply<StreamTaskStatus>> replyTo, String user)
      implements Command {}

  private void runFlow(Iterator<? extends DataProvider.Data> iterator) {
    logger.debug(
        "The task stream was asked to run {} by user {}", taskId, streamTaskDescription.getUser());
  ...
  private Source<? extends DataProvider.Data, NotUsed> getSource(
      Iterator<? extends DataProvider.Data> iterator) {
  ...
  private Source<DataProvider.Data, NotUsed> addFlowStep(
      Source<? extends DataProvider.Data, NotUsed> source) {
    return source
        .takeWhile(e -> status.equals(StreamTaskStatus.Status.IN_PROGRESS))
        .mapAsync(
            parallelism,
            data ->
