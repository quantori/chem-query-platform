...
import static com.quantori.cqp.core.source.SourceRootActor.rootActorsKey;
...
public class StreamTaskServiceImpl implements StreamTaskService {
  ...
  @Override
  public CompletionStage<StreamTaskStatus> processTask(
      StreamTaskDescription task, String flowId, int parallelism, int buffer) {
    ...
    return stage
        .thenCompose(
            actorRef ->
                taskPersistenceService.startTaskCommand(
                    actorRef, task, flowId, parallelism, buffer))
        .thenCompose(
            status -> {
  ...
  @Override
  public CompletionStage<StreamTaskStatus> processTaskFlowAsTask(
      List<StreamTaskDescription> tasks, String type, Consumer<Boolean> onComplete, String user) {
    return processTaskFlowAsTask(
        tasks, type, onComplete, user, StreamTaskActor.PARALLELISM, StreamTaskActor.BUFFER_SIZE);
  }

  @Override
  public CompletionStage<StreamTaskStatus> processTaskFlowAsTask(
      List<StreamTaskDescription> tasks,
      String type,
    ...
    CompletionStage<ActorRef<StreamTaskActor.Command>> stage =
        AskPattern.askWithStatus(
            rootActorRef,
            ref ->
                new TaskServiceActor.CreateFlow(
                    ref, type, this, taskPersistenceServiceSupplier.get()),
            Duration.ofMinutes(1),
            actorSystem.scheduler());

    return stage
        .thenCompose(
            actorRef -> startFlowCommand(actorRef, tasks, onComplete, user, parallelism, buffer))
        .thenCompose(
            status -> {
              if (!status.status().equals(StreamTaskStatus.Status.COMPLETED)
                  && !status.status().equals(StreamTaskStatus.Status.COMPLETED_WITH_ERROR)
  ...
  private CompletionStage<StreamTaskStatus> waitAvailableActorRef(StreamTaskStatus status) {
    ...
    while (count < RETRY_COUNT && !allFound) {
      try {
        allFound =
            checkAllNodesReferences(status.taskId()).toCompletableFuture().get(1, TimeUnit.MINUTES);
      } catch (InterruptedException e) {
        logger.error(
            "The method 'waitAvailableActorRef' was interrupted for task {}", status.taskId(), e);
        Thread.currentThread().interrupt();
        count = RETRY_COUNT;
      } catch (TimeoutException e) {
        logger.warn(
            "Timeout error was caught in the request for a task reference {}", status.taskId());
      } catch (ExecutionException e) {
        logger.warn(
            "Error was found in the request for a task reference {}: {}",
            status.taskId(),
            e.getMessage());
      }
      if (!allFound) {
        try {
          ...
          logger.error(
              "The method 'waitAvailableActorRef' was interrupted in sleep for task {}",
              status.taskId(),
              e);
          Thread.currentThread().interrupt();
          count = RETRY_COUNT;
    ...
    if (allFound) {
      ...
      return CompletableFuture.completedFuture(
          new StreamTaskStatus(status.taskId(), StreamTaskStatus.Status.COMPLETED_WITH_ERROR, 0));
    }
  }
  ...
  CompletableFuture<Boolean> checkAllNodesReferences(String taskId) {
    ...
    return cf.toCompletableFuture()
        .thenCompose(
            listing -> {
              var results =
                  listing.getServiceInstances(rootActorsKey).stream()
                      .map(
                          rootRef ->
                              AskPattern.<SourceRootActor.Command, Boolean>askWithStatus(
                                      ...
                                      ref ->
                                          new SourceRootActor.CheckActorReference(
                                              ref, StreamTaskActor.Command.class, taskId),
                                      Duration.ofMinutes(1),
                                      actorSystem.scheduler())
                                  .toCompletableFuture())
                      .toList();
  ...
  public CompletionStage<StreamTaskStatus> cancelTask(String taskId, String user) {
    return getTaskActorRef(taskId).thenCompose(ref -> requestCancel(ref, user));
  }
        ...
        .thenCompose(
            listing -> {
              if (!listing.getServiceInstances(userKey).isEmpty()) {
                var results =
                    listing.getServiceInstances(userKey).stream()
  ...
  private CompletionStage<StreamTaskStatus> startFlowCommand(
    ...
    return AskPattern.askWithStatus(
        actorRef,
        replyTo ->
            new TaskFlowActor.StartFlow(
                replyTo, streamTaskDescriptions, onComplete, user, parallelism, buffer),
        Duration.ofMinutes(1),
        actorSystem.scheduler());
  }
  ...
  private CompletionStage<ActorRef<StreamTaskActor.Command>> getTaskActorRefWithRetry(
  ...
  private ActorRef<StreamTaskActor.Command> findTaskRefWithTimeout(String taskId, int retryCount) {
    try {
      Thread.sleep(TIMEOUT_MILLS);
      return getTaskActorRefWithRetry(taskId, retryCount)
          .toCompletableFuture()
          .get(1, TimeUnit.MINUTES);
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
      throw new StreamTaskProcessingException(
          "The method findTaskRefWithTimeout was interrupted.", e);
    } catch (TimeoutException e) {
      throw new StreamTaskProcessingException(
          "Timeout error was caught to fetch task actor reference: " + taskId);
    } catch (ExecutionException e) {
      throw new StreamTaskProcessingException(
          "Error was caught to fetch task actor reference: " + taskId, e);
    }
  }

  private CompletionStage<StreamTaskStatus> requestStatus(
      ActorRef<StreamTaskActor.Command> actorRef, String user) {
    return AskPattern.askWithStatus(
        actorRef,
  ...
  private CompletionStage<StreamTaskResult> requestTaskResult(
      ActorRef<StreamTaskActor.Command> actorRef, String user) {
    return AskPattern.askWithStatus(
        actorRef,
  ...
  private CompletionStage<StreamTaskDetails> requestTaskDetails(
      ActorRef<StreamTaskActor.Command> actorRef, String user) {
    return AskPattern.askWithStatus(
        actorRef,
  ...
  private CompletionStage<StreamTaskStatus> requestCancel(
      ActorRef<StreamTaskActor.Command> actorRef, String user) {
    return AskPattern.askWithStatus(
        actorRef,
